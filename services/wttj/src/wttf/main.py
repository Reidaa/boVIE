...
def collect(
    ...
    *,
    ...
    scan = hashlib.sha256(
        json.dumps([query, countries, contracts, limit, max_pages]).encode()
    ).hexdigest()
    inspected = 0
    ...
    for page in range(1, max_pages + 1):
        ...
        for hit in result.data[: limit - inspected]:
            ...
            if job.status != "published":
                continue
            if contracts and job.contract_type not in contracts:
                continue
...
@click.option(
    ...
    default="",
    envvar="WTTJ_QUERY",
    help="Optional job title search; empty includes all titles.",
)
@click.option(
    "--limit",
    default=50,
...
@click.option(
    ...
    "contracts",
    multiple=True,
    envvar="WTTJ_CONTRACTS",
    help="Optional WTTJ contract type (repeatable), e.g. full_time or internship.",
)
def main(
    query: str,
    limit: int,
    ...
    """Discover published jobs of any contract type on Welcome to the Jungle."""
    logger.remove()
    ...
    try:
        with httpx.Client(base_url=API_URL, timeout=15) as client:
            collect(
                ...
                contracts=tuple(contract.lower() for contract in contracts),
            )
